...
// we count on FP exceptions being turned off to avoid range problems
    ...
    //int signed_zwidth = d_zwidth;
    //int signed_screenwidth = r_screenwidth;
	...
	do
	{
        ...
        //s_spanletvars.pdest   = (byte *)d_viewbuffer + (vid_width * pspan->v) + pspan->u;
        //s_spanletvars.pz      = d_pzbuffer + (vid_width * pspan->v) + pspan->u;
		...
		if (count <= 0)
        //if (count <= 0 || pspan->u < 0 || pspan->v < 0)// additional checks for finding bug!
        {
            goto NextSpan;
        }
	...
	// we count on FP exceptions being turned off to avoid range problems
		...
		// calculate s and t at the far end of the span
            ...
            //if ( !iswater )// hack! disable water rendering
            if (pspan->v >= 0)// water rendering can have negative coordinates!
            {
...
// transform vertices into viewspace and project
	...
	for (i=0 ; i<nump ; i++)
	{
        ...
        if (pout->v < 0)// why can it be negative for water surfaces?
        {
            // hack!
            pout->v = 0;
        }

        // sizeof(pv) != sizeof(float)
        // pv == float*
        // "worked" only on 32bit pointers!
        //
