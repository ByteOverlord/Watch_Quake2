...
void SV_WipeSavegame (char *savename)
{
	...
	Com_sprintf (name, sizeof(name), "%s/save/%s/server.ssv", FS_Savedir (), savename);
	remove (name);
	Com_sprintf (name, sizeof(name), "%s/save/%s/game.ssv", FS_Savedir (), savename);
	remove (name);

	Com_sprintf (name, sizeof(name), "%s/save/%s/*.sav", FS_Savedir (), savename);
	...
	Sys_FindClose ();
	Com_sprintf (name, sizeof(name), "%s/save/%s/*.sv2", FS_Savedir (), savename);
	s = Sys_FindFirst(name, 0, 0 );
	while (s)
	{
		remove (s);
...
void SV_CopySaveGame (char *src, char *dst)
{
	...
	Com_sprintf (name, sizeof(name), "%s/save/%s/server.ssv", FS_Savedir(), src);
	Com_sprintf (name2, sizeof(name2), "%s/save/%s/server.ssv", FS_Savedir(), dst);
	FS_CreatePath (name2);
	CopyFile (name, name2);

	Com_sprintf (name, sizeof(name), "%s/save/%s/game.ssv", FS_Savedir(), src);
	Com_sprintf (name2, sizeof(name2), "%s/save/%s/game.ssv", FS_Savedir(), dst);
	CopyFile (name, name2);

	Com_sprintf (name, sizeof(name), "%s/save/%s/", FS_Savedir(), src);
	len = strlen(name);
	Com_sprintf (name, sizeof(name), "%s/save/%s/*.sav", FS_Savedir(), src);
	found = Sys_FindFirst(name, 0, 0 );
	while (found)
	{
		strcpy (name+len, found+len);

		Com_sprintf (name2, sizeof(name2), "%s/save/%s/%s", FS_Savedir(), dst, found+len);
		CopyFile (name, name2);
...
void SV_WriteLevelFile (void)
{
	...
	Com_sprintf (name, sizeof(name), "%s/save/current/%s.sv2", FS_Savedir(), sv.name);
	f = fopen(name, "wb");
	if (!f)
	{
	...
	Com_sprintf (name, sizeof(name), "%s/save/current/%s.sav", FS_Savedir(), sv.name);
	ge->WriteLevel (name);
}
...
void SV_WriteServerFile (qboolean autosave)
{
	...
	Com_sprintf (name, sizeof(name), "%s/save/current/server.ssv", FS_Savedir());
	f = fopen (name, "wb");
	if (!f)
	{
	...
	// write game state
	Com_sprintf (name, sizeof(name), "%s/save/current/game.ssv", FS_Savedir());
...
void SV_GameMap_f (void)
{
	...
	FS_CreatePath (va("%s/save/current/", FS_Savedir()));
...
void SV_Loadgame_f (void)
{
	...
	Com_sprintf (name, sizeof(name), "%s/save/%s/server.ssv", FS_Savedir(), Cmd_Argv(1));
	f = fopen (name, "rb");
	if (!f)
	{
