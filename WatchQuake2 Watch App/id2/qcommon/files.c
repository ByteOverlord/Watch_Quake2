...
char    fs_bundledir[MAX_OSPATH];
char    fs_savedir[MAX_OSPATH];
...
void FS_SetConfigOverride(int i)
{
    config_override = i;
}
...
int FS_FOpenFile (char *filename, FILE **file)
{
    ...
    //printf("open file %s\n",filename);
    if (!config_override && !strcmp(filename,"config.cfg"))
    {
        Com_sprintf (netpath, sizeof(netpath), "%s/%s",fs_savedir, filename);
        *file = fopen (netpath, "rb");
        if (*file)
        {
            Com_DPrintf ("saved file: %s\n",netpath);
