...
void S_PaintChannels(int endtime)
{
    ...
    int bgm_vol = bgm_volume->value*256;
...
//			Com_Printf ("clear\n");
		...
		{	// copy from the streaming sound source
            ...
            // replace with CDAudio_MixSamples style mixer?
	...
	// paint in the channels.
		...
		for (i=0; i<MAX_CHANNELS ; i++, ch++)
		{
			...
			while (ltime < end)
			{
                ...
                sc = ch->sfx->cache;//S_LoadSound (ch->sfx);
                if (!sc)
                    break;
...
void S_InitScaletable (void)
{
    ...
    for (i=0 ; i<32 ; i++)
    {
        scale = i * 8 * 256 * s_volume->value * SFX_VOLUME_MULTIPLIER;
        for (j=0 ; j<256 ; j++)
        {
...
extern float q2DeltaTimeF;

float clamp(float fmin, float f, float fmax)
{
    return f < fmin ? fmin : f > fmax ? fmax : f;
}

float q_min(float fmin, float f)
{
    return f < fmin ? f : fmin;
}

float q_max(float fmax, float f)
{
    return f > fmax ? f : fmax;
}

void S_SetUnderwaterIntensity (float target)
{
    cvar_t snd_waterfx;
    snd_waterfx.value = 1.0f;

    target *= clamp(0.f, snd_waterfx.value, 2.f);
    if (underwater.intensity < target)
    {
        underwater.intensity += q2DeltaTimeF * 4.f;
