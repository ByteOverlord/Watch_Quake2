...
void M_DrawCustomLabel (int cx, int cy, char *str)
{
    while (*str)
    {
        re.DrawChar(cx, cy, (*str)+128);
        str++;
        cx += 8;
    }
}
...
void M_Main_Draw (void)
{
    ...
    //M_DrawCustomLabel(xoffset, ystart + 2 * 40 + 13 + 8,"[ CHEATS ]");
    //M_DrawCustomLabel(xoffset, ystart + 3 * 40 + 13 + 8,"[ MAPS   ]");

	M_DrawCursor( xoffset - 25, ystart + m_main_cursor * 40 + 11, (int)(cls.realtime / 100)%NUM_CURSOR_FRAMES );
    //M_DrawCursor( xoffset - 25, ystart + m_main_cursor * 40 + 11, (int)(Sys_Milliseconds() / 100)%NUM_CURSOR_FRAMES );
...
void M_Menu_Cheats_Init(void)
{
    ...
    s_cheats_giveall_action.generic.y        = 10;
...
{
    M_Menu_Cheats_Init();
    M_PushMenu(M_Cheats_Draw,M_Cheats_Key);
}

static menuframework_s s_maps_menu;
static menulist_s s_maps_mapselect_action;
static menulist_s s_maps_skillselect_action;
static menuaction_s s_maps_mapload_action;
...
static void Map_SkillSelectFunc(void* unused)
{
    
}

static void Map_LoadMapFunc(void* unused)
{
    Cvar_ForceSet( "skill",skillNums[s_maps_skillselect_action.curvalue]);
    //SV_Map(qFalse,levelNames[s_maps_mapselect_action.curvalue],qFalse);
    //Cbuf_ExecuteText(EXEC_NOW,"killserver");
    char buffer[256];
    snprintf(buffer,256,"gamemap *%s",levelNames[s_maps_mapselect_action.curvalue]);
...
void M_Menu_Maps_Init(void)
{
    ...
    Menu_AddItem(&s_maps_menu,&s_maps_mapselect_action);
    Menu_AddItem(&s_maps_menu,&s_maps_skillselect_action);
    Menu_AddItem(&s_maps_menu,&s_maps_mapload_action);

    Menu_SetStatusBar( &s_maps_menu, NULL );
...
static menulist_s s_options_demoselect_box;
static menuaction_s s_options_demoload_action;
static menuaction_s s_options_demoreset_action;
static menuaction_s		s_options_defaults_action;
//static menuaction_s		s_options_customize_options_action;
static menuslider_s		s_options_sensitivity_slider;
static menuslider_s     s_options_forwardspeed_slider;
static menuslider_s     s_options_sidespeed_slider;
static menuslider_s     s_options_friction_slider;
...
static menuslider_s     s_options_bgmvolume_slider;
static menuslider_s     s_options_cinvolume_slider;
static menuslider_s     s_options_gamma_slider;
//static menulist_s		s_options_joystick_box;
//static menulist_s		s_options_cdvolume_box;
//static menulist_s		s_options_quality_list;
//static menulist_s		s_options_compatibility_list;
//static menulist_s		s_options_console_action;
static menulist_s s_options_fps_toggle;
static menulist_s s_options_jumpcrouch_toggle;

static int changed_settings = 0;
...
{
	Cvar_SetValue( "sensitivity", s_options_sensitivity_slider.curvalue / 4.0F );
    changed_settings = 1;
}

static void ForwardSpeedFunc( void *unused )
{
    Cvar_SetValue( "joy_forwardsensitivity", s_options_forwardspeed_slider.curvalue / 4.0F );
    changed_settings = 1;
}

static void SideSpeedFunc( void *unused )
{
    Cvar_SetValue( "joy_sidesensitivity", s_options_sidespeed_slider.curvalue / 4.0F );
    changed_settings = 1;
}

static void FrictionSpeedFunc( void *unused )
{
    Cvar_SetValue( "movement_friction", s_options_friction_slider.curvalue / 4.0F );
    changed_settings = 1;
}
...
static void ControlsSetMenuItemValues( void )
{
    ...
    s_options_bgmvolume_slider.curvalue     = Cvar_VariableValue( "bgm_volume" ) * 10;
    s_options_cinvolume_slider.curvalue     = Cvar_VariableValue( "cin_volume" ) * 10;
	//s_options_cdvolume_box.curvalue 		= !Cvar_VariableValue("cd_nocd");
	//s_options_quality_list.curvalue			= !Cvar_VariableValue( "s_loadas8bit" );
	s_options_sensitivity_slider.curvalue	= Cvar_VariableValue( "sensitivity" ) * 4;
    
    s_options_gamma_slider.curvalue      = Cvar_VariableValue( "vid_gamma" ) * 10;
    
    s_options_forwardspeed_slider.curvalue = Cvar_VariableValue( "joy_forwardsensitivity") * 4;
    s_options_sidespeed_slider.curvalue = Cvar_VariableValue( "joy_sidesensitivity") * 4;
    s_options_friction_slider.curvalue = Cvar_VariableValue( "movement_friction") * 4;
    
    s_options_jumpcrouch_toggle.curvalue = Cvar_VariableValue("cl_jumpandcrouch");
...
{
    FS_SetConfigOverride(1);
	Cbuf_AddText ("exec config.cfg\n");//"exec default.cfg\n"
	Cbuf_Execute();
    FS_SetConfigOverride(0);
    changed_settings = 1;
...
static void UpdateBGMVolumeFunc( void *unused )
{
    Cvar_SetValue( "bgm_volume", s_options_bgmvolume_slider.curvalue / 10 );
    changed_settings = 1;
}

static void UpdateCINVolumeFunc( void *unused )
{
    Cvar_SetValue( "cin_volume", s_options_cinvolume_slider.curvalue / 10 );
    changed_settings = 1;
}

static void UpdateGammaFunc( void *unused )
{
    Cvar_SetValue( "vid_gamma", s_options_gamma_slider.curvalue / 10 );
    changed_settings = 1;
}
...
static void WQ_FPSFunc( void *unused )
{
    Cvar_SetValue( "cl_showfps", s_options_fps_toggle.curvalue );
}

static void WQ_JumpAndCrouchSwapFunc( void *unused )
{
    Cvar_SetValue( "cl_jumpandcrouch", s_options_jumpcrouch_toggle.curvalue );
    changed_settings = 1;
}
...
static void Map_DemoSelectFunc(void* unused)
{
...
static void Map_DemoLoadFunc(void* unused)
{
    //char buffer[256];
    //snprintf(buffer,256,"killserver ; demomap %s",demoNames[s_options_demoselect_box.curvalue]);
    //Cbuf_ExecuteText(EXEC_NOW,buffer);
    Cbuf_ExecuteText(EXEC_NOW,"killserver");
    SV_Map(qTrue,demoNames[s_options_demoselect_box.curvalue],qFalse);
    M_ForceMenuOff();
    WQOnBenchmarkBegin(4,s_options_demoselect_box.curvalue == 0 ? 65 : 60);
}

static void Map_DemoResetFunc(void* unused)
{
    WQOnBenchmarkClear();
}
...
void Options_MenuInit( void )
{
	...
	*/
    ...
    s_options_demoselect_box.generic.type = MTYPE_SPINCONTROL;
    s_options_demoselect_box.generic.x    = 0;
    s_options_demoselect_box.generic.y    = 0;
    s_options_demoselect_box.generic.name    = "demo select";
    s_options_demoselect_box.generic.callback    = Map_DemoSelectFunc;
    s_options_demoselect_box.itemnames = demoNames;

    s_options_demoload_action.generic.type = MTYPE_ACTION;
    s_options_demoload_action.generic.x = 0;
    s_options_demoload_action.generic.y = 10;
    s_options_demoload_action.generic.name = "demo load";
    s_options_demoload_action.generic.callback = Map_DemoLoadFunc;

    s_options_demoreset_action.generic.type = MTYPE_ACTION;
    s_options_demoreset_action.generic.x = 0;
    s_options_demoreset_action.generic.y = 20;
    s_options_demoreset_action.generic.name = "demo reset";
    s_options_demoreset_action.generic.callback = Map_DemoResetFunc;

	s_options_sfxvolume_slider.generic.type	= MTYPE_SLIDER;
	s_options_sfxvolume_slider.generic.x	= 0;
	s_options_sfxvolume_slider.generic.y	= 30;
    ...
    s_options_bgmvolume_slider.generic.type    = MTYPE_SLIDER;
    s_options_bgmvolume_slider.generic.x    = 0;
    s_options_bgmvolume_slider.generic.y    = 40;
    s_options_bgmvolume_slider.generic.name    = "music volume";
    s_options_bgmvolume_slider.generic.callback    = UpdateBGMVolumeFunc;
    s_options_bgmvolume_slider.minvalue        = 0;
    s_options_bgmvolume_slider.maxvalue        = 10;
    //s_options_bgmvolume_slider.curvalue        = Cvar_VariableValue( "bgm_volume" ) * 10;

    s_options_cinvolume_slider.generic.type    = MTYPE_SLIDER;
    s_options_cinvolume_slider.generic.x    = 0;
    s_options_cinvolume_slider.generic.y    = 50;
    s_options_cinvolume_slider.generic.name    = "cinematics volume";
    s_options_cinvolume_slider.generic.callback    = UpdateCINVolumeFunc;
    s_options_cinvolume_slider.minvalue        = 0;
    s_options_cinvolume_slider.maxvalue        = 10;

    s_options_gamma_slider.generic.type    = MTYPE_SLIDER;
    s_options_gamma_slider.generic.x    = 0;
    s_options_gamma_slider.generic.y    = 60;
    s_options_gamma_slider.generic.name    = "Gamma";
    s_options_gamma_slider.generic.callback    = UpdateGammaFunc;
    s_options_gamma_slider.minvalue        = 0;
    s_options_gamma_slider.maxvalue        = 10;
    //s_options_gamma_slider.curvalue        = Cvar_VariableValue( "vid_gamma" ) * 10;
    ...
    s_options_forwardspeed_slider.generic.type    = MTYPE_SLIDER;
    s_options_forwardspeed_slider.generic.x        = 0;
    s_options_forwardspeed_slider.generic.y        = 70;
    s_options_forwardspeed_slider.generic.name    = "forward speed";
    s_options_forwardspeed_slider.generic.callback = ForwardSpeedFunc;
    s_options_forwardspeed_slider.minvalue        = 1;
    s_options_forwardspeed_slider.maxvalue        = 8;
    
    s_options_sidespeed_slider.generic.type    = MTYPE_SLIDER;
    s_options_sidespeed_slider.generic.x        = 0;
    s_options_sidespeed_slider.generic.y        = 80;
    s_options_sidespeed_slider.generic.name    = "side speed";
    s_options_sidespeed_slider.generic.callback = SideSpeedFunc;
    s_options_sidespeed_slider.minvalue        = 1;
    s_options_sidespeed_slider.maxvalue        = 8;

    s_options_friction_slider.generic.type    = MTYPE_SLIDER;
    s_options_friction_slider.generic.x        = 0;
    s_options_friction_slider.generic.y        = 90;
    s_options_friction_slider.generic.name    = "friction";
    s_options_friction_slider.generic.callback = FrictionSpeedFunc;
    s_options_friction_slider.minvalue        = 1;
    s_options_friction_slider.maxvalue        = 8;

	s_options_sensitivity_slider.generic.type	= MTYPE_SLIDER;
	s_options_sensitivity_slider.generic.x		= 0;
	s_options_sensitivity_slider.generic.y		= 100;
	s_options_sensitivity_slider.generic.name	= "turn speed";
	s_options_sensitivity_slider.generic.callback = MouseSpeedFunc;
	s_options_sensitivity_slider.minvalue		= 1;
	s_options_sensitivity_slider.maxvalue		= 8;
	...
	s_options_lookstrafe_box.generic.y	= 130;
...
*/
    ...
    s_options_jumpcrouch_toggle.generic.type    = MTYPE_SPINCONTROL;
    s_options_jumpcrouch_toggle.generic.x        = 0;
    s_options_jumpcrouch_toggle.generic.y        = 150;
    s_options_jumpcrouch_toggle.generic.name    = "swap jump and crouch";
    s_options_jumpcrouch_toggle.generic.callback = WQ_JumpAndCrouchSwapFunc;
    s_options_jumpcrouch_toggle.itemnames = yesno_names;

    s_options_fps_toggle.generic.type    = MTYPE_SPINCONTROL;
    s_options_fps_toggle.generic.x        = 0;
    s_options_fps_toggle.generic.y        = 160;
    s_options_fps_toggle.generic.name    = "show fps";
    s_options_fps_toggle.generic.callback = WQ_FPSFunc;
    s_options_fps_toggle.itemnames = yesno_names;
    ...
    Menu_AddItem( &s_options_menu, ( void * ) &s_options_demoselect_box );
    Menu_AddItem( &s_options_menu, ( void * ) &s_options_demoload_action );
    Menu_AddItem( &s_options_menu, ( void * ) &s_options_demoreset_action );
	Menu_AddItem( &s_options_menu, ( void * ) &s_options_sfxvolume_slider );
    Menu_AddItem( &s_options_menu, ( void * ) &s_options_bgmvolume_slider );
    Menu_AddItem( &s_options_menu, ( void * ) &s_options_cinvolume_slider );
    Menu_AddItem( &s_options_menu, ( void * ) &s_options_gamma_slider );
	//Menu_AddItem( &s_options_menu, ( void * ) &s_options_cdvolume_box );
	//Menu_AddItem( &s_options_menu, ( void * ) &s_options_quality_list );
	//Menu_AddItem( &s_options_menu, ( void * ) &s_options_compatibility_list );
    Menu_AddItem( &s_options_menu, ( void * ) &s_options_forwardspeed_slider );
    Menu_AddItem( &s_options_menu, ( void * ) &s_options_sidespeed_slider );
    Menu_AddItem( &s_options_menu, ( void * ) &s_options_friction_slider );
    ...
    Menu_AddItem( &s_options_menu, ( void * ) &s_options_jumpcrouch_toggle );
    Menu_AddItem( &s_options_menu, ( void * ) &s_options_fps_toggle );
...
void Create_Savestrings (void)
{
	...
	{
		Com_sprintf (name, sizeof(name), "%s/save/save%i/server.ssv", FS_Savedir(), i);
		f = fopen (name, "rb");
		if (!f)
		{
