...
#define LAYOUT_STATS   0
#define LAYOUT_DEFAULT 1

void SCR_ExecuteLayoutString (char *s, int layoutID)
{
	int		x, y;
	int		value;
	char	*token;
...
{
	SCR_ExecuteLayoutString (cl.configstrings[CS_STATUSBAR],LAYOUT_STATS);
}
...
void SCR_DrawLayout (void)
{
	if (!cl.frame.playerstate.stats[STAT_LAYOUTS])
		return;
	SCR_ExecuteLayoutString (cl.layout,LAYOUT_DEFAULT);
}
