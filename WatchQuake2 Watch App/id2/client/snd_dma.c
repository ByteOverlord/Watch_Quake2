...
void S_Init (void)
{
	...
	else
	{
        ...
        bgm_volume = Cvar_Get ("bgm_volume", "0.25", CVAR_ARCHIVE);
        cin_volume = Cvar_Get("cin_volume","0.25",CVAR_ARCHIVE);
...
====================
*/
void S_Batch_StartSound(vec3_t origin, int entnum, int entchannel, sfx_t *sfx, float fvol, float attenuation, float timeofs)
{
	sfxcache_t	*sc;
	int			vol;
	playsound_t	*ps, *sort;
...
void S_StartSound(vec3_t origin, int entnum, int entchannel, sfx_t *sfx, float fvol, float attenuation, float timeofs)
{
    ...
    if (origin)
    {
        ...
        call->flags |= SNDCALL_FLAG_ORIGIN;
    }
    call->fvol = fvol;
    call->attenuation = attenuation;
    call->timeofs = timeofs;
}
...
void S_BatchProcessing(void)
{
    for (int i=0; i<sndcall_cache_size; i++)
    {
        ...
        {
            S_Batch_StartSound((call->flags & SNDCALL_FLAG_ORIGIN) ? call->origin : NULL,call->entnum,call->entchannel,call->sfx,call->fvol,call->attenuation,call->timeofs);
        }
...
void S_RawSamples (int samples, int rate, int width, int channels, byte *data)
{
    ...
    int cin_vol = cin_volume->value*256;
    cin_vol = cin_vol < 0 ? 0 : cin_vol > 256 ? 256 : cin_vol;
    // video audio 22050 Hz, 16bit, stereo
...
//Com_Printf ("%i < %i < %i\n", soundtime, paintedtime, s_rawend);
	if (channels == 2 && width == 2)
	{
		...
		{	// optimized case
			for (i=0 ; i<samples ; i++)
			{
                ...
                s_rawsamples[dst].left =
                    LittleShort(((short *)data)[i*2]) * cin_vol;
                s_rawsamples[dst].right =
                    LittleShort(((short *)data)[i*2+1]) * cin_vol;
			}
		}
		else
		{
			for (i=0 ; ; i++)
			{
                ...
                s_rawsamples[dst].left =
                LittleShort(((short *)data)[src*2]) * cin_vol;
                s_rawsamples[dst].right =
                LittleShort(((short *)data)[src*2+1]) * cin_vol;
			}
		}
	}
	else if (channels == 1 && width == 2)
	{
		for (i=0 ; ; i++)
		{
			...
			s_rawsamples[dst].right =
			    LittleShort(((short *)data)[src]) << 8;*/
            s_rawsamples[dst].left =
                LittleShort(((short *)data)[src]) * cin_vol;
            s_rawsamples[dst].right =
                LittleShort(((short *)data)[src]) * cin_vol;
		}
	}
	else if (channels == 2 && width == 1)
	{
		for (i=0 ; ; i++)
		{
            ...
            s_rawsamples[dst].left =
                (((char *)data)[src*2] << 8) * cin_vol;
            s_rawsamples[dst].right =
                (((char *)data)[src*2+1] << 8) * cin_vol;
		}
	}
	else if (channels == 1 && width == 1)
	{
		for (i=0 ; ; i++)
		{
            ...
            s_rawsamples[dst].left =
                ((((byte *)data)[src]-128) << 8) * cin_vol;
            s_rawsamples[dst].right = ((((byte *)data)[src]-128) << 8) * cin_vol;
		}
	}
    SND_UNLOCK
}
...
============
*/
void S_Update(vec3_t origin, vec3_t forward, vec3_t right, vec3_t up, int flags)
{
	int			i;
	int			total;
	channel_t	*ch;
    ...
    // todo
    // implement S_UnderwaterIntensityForContents
    
    // mleaf_t *l;
    // l = Mod_PointInLeaf (listener_origin, cl.worldmodel);
    // float intensity = S_UnderwaterIntensityForContents(l->contents)
    S_SetUnderwaterIntensity((flags & RDF_UNDERWATER) ? 1.0f : 0.0f);
	//
	// debugging output
	//
	if (s_show->value)
