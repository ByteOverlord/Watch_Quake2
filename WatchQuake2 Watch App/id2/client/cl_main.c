...
//	cl_minfps = Cvar_Get ("cl_minfps", "5", 0);
    ...
    movefriction = Cvar_Get ("movement_friction", "1", CVAR_ARCHIVE);
...
{
	FILE	*f;
	char	path[MAX_OSPATH];// was MAX_QPATH

	if (cls.state == ca_uninitialized)
		return;

    Com_sprintf (path, sizeof(path),"%s/config.cfg",FS_Savedir());//FS_Gamedir());
	f = fopen (path, "w");
	if (!f)
	{
...
void CL_Frame (int msec)
{
	...
	cl.time += extratime;
    cls.realtime = Sys_Milliseconds();//curtime;
	...
	// update audio
	S_Update (cl.refdef.vieworg, cl.v_forward, cl.v_right, cl.v_up, cl.refdef.rdflags);
