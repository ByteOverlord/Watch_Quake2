...
void Con_DrawNotify (void)
{
	...
	for (i= con.current-NUM_CON_TIMES+1 ; i<=con.current ; i++)
	{
		...
		for (x = 0 ; x < con.linewidth ; x++)
			re.DrawChar ( ((x+1)<<3) + NOTIFY_X_OFFSET, v + NOTIFY_Y_OFFSET, text[x]);
	...
	if (cls.key_dest == key_message)
	{
		...
		while(s[x])
		{
			re.DrawChar ( ((x+skip)<<3) + NOTIFY_X_OFFSET, v + NOTIFY_Y_OFFSET, s[x]);
			x++;
		}
		re.DrawChar ( ((x+skip)<<3) + NOTIFY_X_OFFSET, v + NOTIFY_Y_OFFSET, 10+((cls.realtime>>8)&1));
		v += 8;
	}
