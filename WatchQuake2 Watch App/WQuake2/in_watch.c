...
typedef uint32_t* PDWORD;// PDWORD == DWORD* ?
/*
...
void IN_Init (void)
{
    ...
    joy_forwardsensitivity    = Cvar_Get ("joy_forwardsensitivity",    "10",        0);
    joy_sidesensitivity        = Cvar_Get ("joy_sidesensitivity",        "10",        0);
...
===========
*/
float ClampFloat(float f, float fmin, float fmax)
{
...
/*    IN_MouseMove (cmd);
    ...
    in_forwardmove = g_WQForwardSpeed * joy_forwardsensitivity->value;
    in_sidestepmove = g_WQStrafeSpeed * joy_sidesensitivity->value;
    in_forwardmove = ClampFloat(in_forwardmove,-1,1);
    in_sidestepmove = ClampFloat(in_sidestepmove,-1,1);
    /*if (g_WQFlags & 0x01)
    {
    ...
    mouse_x = g_WQTurnX * sensitivity->value * 20;
    mouse_y = g_WQTurnY * sensitivity->value * 20;
    ...
    if (cl.viewangles[PITCH] > maxAngle)// 80
        cl.viewangles[PITCH] = maxAngle;
    if (cl.viewangles[PITCH] < minAngle)// -70
        cl.viewangles[PITCH] = minAngle;

    cmd->forwardmove = in_forwardmove * cl_forwardspeed->value;
    cmd->sidemove = in_sidestepmove * cl_sidespeed->value;
}
